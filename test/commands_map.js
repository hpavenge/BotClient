...
var commands_map = {
    ...
    "card-actioncall": {
        ...
        evalOtherWindow: function (nightmare) {
            return __awaiter(this, void 0, void 0, function* () {
                const windows_fn = yield nightmare['windows'].bind(nightmare);
                const windows = yield windows_fn();
                const new_win = yield windows[1];
    ...
    "carousel-to-right": {
        client: () => new Promise((resolve) => {
            var right_arrow = document.querySelectorAll('.scroll.next')[0];
    ...
    "carousel-to-left": {
        client: () => new Promise((resolve) => {
            var right_arrow = document.querySelectorAll('.scroll.next:not([disabled])')[0];
            // One-Click to the right
            right_arrow.click();
            setTimeout(() => {
                // One-click to the left
                var left_arrow = document.querySelectorAll(".scroll.previous")[0];
                left_arrow.click();
                setTimeout(() => {
    ...
    "document-plain": {
        client: function () {
            var link = document.querySelector('.wc-message-wrapper:last-child .wc-message.wc-message-from-bot a');
            if (!link)
                return false;
    ...
    "markdown-url-needs-encoding": {
        client: function () {
            ...
            for (var i = 0; i < links.length; i++) {
                var link = links[i];
                //check if value is encoded
                if (link.href !== "https://bing.com/?q=some%20value")
                    return false;
            }
            return true;
        },
    ...
    "suggested-actions-away": {
        client: () => new Promise((resolve) => {
            var green_button = document.querySelectorAll('button[title="Green"]')[0];
            green_button.click();
            setTimeout(() => {
                var show_actions_length = document.querySelectorAll('.show-actions').length;
    ...
    "suggested-actions-click": {
        client: () => new Promise((resolve) => {
            var red_button = document.querySelectorAll('button[title="Red"]')[0];
            red_button.click();
            setTimeout(() => {
                // Waiting more time
                setTimeout(() => {
    ...
    "upload": {
        do: function (nightmare) {
            try {
                const upload = nightmare.upload.bind(nightmare);
        ...
        client: function () {
            var img = document.querySelectorAll('.wc-message-wrapper:last-child .wc-message.wc-message-from-bot img')[0];
    ...
    "card CalendarReminder": {
        client: () => new Promise((resolve) => {
            setTimeout(() => {
                var selectPullDown = document.querySelector('.wc-adaptive-card .ac-container select');
    ...
    "speech clicking-mic-starts-speaking": {
        ...
        client: function () {
            debugger;
            return ((document.querySelector('.wc-shellinput').placeholder === 'Listening...'));
        }
    },
    "speech click-mic-click-to-stop": {
        ...
        client: function () {
            return ((document.querySelector('.wc-shellinput').placeholder === 'Type your message...'));
        }
    },
    ...
    "focus on type": {
        ...
        client: function () {
            return ((document.querySelector('.wc-shellinput').value === 'Hi!'));
        }
    },
    "type on Adaptive Cards": {
        ...
        client: function () {
            return ((document.querySelector('.ac-input').value === 'John Doe'));
        }
    },
...
if (testOnly && testOnly.length > 0) {
    for (var key in commands_map)
        if (testOnly.indexOf(key) < 0)
            delete commands_map[key];
}
